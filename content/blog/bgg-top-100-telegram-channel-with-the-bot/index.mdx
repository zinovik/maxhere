...
<br />
<br />
Feel free to subscribe to get these updates every Saturday at
9:00 AM UTC ;)
...
```typescript
  if (process.env.TELEGRAM_TOKEN === undefined) {
    throw new ConfigParameterNotDefinedError(
      'TELEGRAM_TOKEN',
    );
  }
...
```typescript
    ...
    const namesYears = select(
      GAME_NAMES_YEARS_X_PATH,
      dom,
    ).map(selectedValue =>
      selectedValue.textContent.trim(),
    );
...
```typescript
    ...
    const oldDate = this.getDateString(
      'Changes from date',
      oldData.date,
    );
```
...
```typescript
    const newGames: Game[] = [];
    const droppedGames: Game[] = oldData.games.filter(
      oldGame =>
        newData.games.every(
          newGame => newGame.name !== oldGame.name,
        ),
    );
    const increaseGames: {
      games: Game[];
...
```typescript
    const increaseGamesString = this.getAdditionalList(
      `⬆️ Highest ranking increase${
        increaseGames.change > 0
          ? ` (+${increaseGames.change})`
          : ''
      }`,
      increaseGames.games,
    );
    const decreaseGamesString = this.getAdditionalList(
      `⬇️ Highest ranking decrease${
        decreaseGames.change < 0
          ? ` (${decreaseGames.change})`
          : ''
      }`,
      decreaseGames.games,
    );
